...
//         {
//             string input = "isahgkjlasdhkjlsDHFKAJSDFASDF";
...
//                     charArray[i + 1] != charArray[i + 4] &&
//                     charArray[i + 2] != charArray[i] &&
//                     charArray[i + 2] != charArray[i + 1] &&
//                     charArray[i + 2] != charArray[i + 3] &&
//                     charArray[i + 2] != charArray[i + 4] &&
//                     charArray[i + 3] != charArray[i] &&
//                     charArray[i + 3] != charArray[i + 1] &&
//                     charArray[i + 3] != charArray[i + 2] &&
...
//             if (found)
//             {
//                 int end = count + 4; // add 4 to the count to find the end of the block
//                 Console.WriteLine($"The end of the block of 4 characters that are not the same can be found after {end} characters.");
//             }
//             else
//             {
//                 Console.WriteLine("No block of 4 characters that are not the same was found.");
//             }
//         }
//     }
// }
...
//2
//Einmal Senf zum Würschtelprogramm von Part 1, weil des kann man hier nimmer hernhemen.
...
class Program
{
    static void Main(string[] args)
    {
        ...
        for (int i = 0; i < charArray.Length - 14; i++)
        {
            char[] subArray = new char[14];
            for (int y = 0; y < 14; y++)
            {
                subArray[y] = charArray[y + i];
            }
            string inputs = new string(subArray);
            var counts = inputs.GroupBy(c => c).ToDictionary(g => g.Key, g => g.Count());

            if (counts.Where(c => c.Value > 1).Any())
            {
                // Console.WriteLine("Nope" + (i));
            }
            else
            {
                Console.WriteLine("Index at: " + (i + 14));
                // for some reason i have to add 14 to this, if you know why, feel free to fix this code.. 
                break;
